...
RUN find /root/licheepi-nano/ -type f ! \( -name '*.bmp' -o -name '*.smc' \) -exec sed -i 's/\r$//' {} \;
...
#RUN BR2_EXTERNAL=/root/licheepi-nano make zlib-install

RUN BR2_EXTERNAL=/root/licheepi-nano make snes9x

#RUN ls -al /root/buildroot/output/build/snes9x-1.53.sdl/sdl/
